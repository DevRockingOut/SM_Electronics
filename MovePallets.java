...
class MovePallets extends Activity {
	...
	static int[] canMovePosi=new int[model.totalPallets];
	static int[] canMovePosj=new int[model.totalPallets];
	static int m=0;
	//check if there is available space ahead of the pallets
	for(int i=Initialise.C1; i<=Initialise.C8; i++) {
	   for(int j=0; j<=8; j++) {
		  if (PowerAndFreeConveyor.PowerndFreeConv[i].pos[j]==null)
		  {
			  canMovePosi[m]=i;
		      canMovePosj[m]=j;
		      m++;
